...
for _ in range(n_iters):
    for n_cls in n_classes:
        common_task_args = {'n_classes': n_cls, 'n_points': n_points, 'bursty': bursty, 'n_labels': n_labels, 'seed': new_seed()}
        common_train_args = {'train_iters': train_iters, 'test_iters': 1, 'test_every': 1000, 'loss': 'ce'}

        cases = [
            Case('MLP', MlpConfig(n_out=n_labels, n_layers=3, n_hidden=512), train_args=common_train_args),
            Case('Transformer', TransformerConfig(n_out=n_labels, n_layers=3, n_heads=4, n_hidden=512, n_mlp_layers=3), train_args=common_train_args),
            # Case('MNN (IWL)', PolyConfig(n_out=n_labels, n_layers=1, n_hidden=512), train_args=common_train_args),
        ]

        for case in cases:
            case.train_task = GautamMatch(**common_task_args)
...
for case in all_cases:
    ...
    case.state = None # for memory reasons
...
def old_run_icl_iwl_comparison():
    n_iters = 3
    train_iters = 150_000
    n_labels = 32
